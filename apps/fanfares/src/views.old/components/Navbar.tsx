...
export function Navbar() {
  ...
  const { balance, publicKey, currentCreatorHasUserAccount, playerMediaKey } = useAppState();
  ...
  const needHelp = () => {
    ...
    router.push(`/support?startingTab=ContactUs&tick=${Keypair.generate().publicKey.toString()}`);
  };
  ...
  const isCurrentLink = (href: string) => {
    return (currentPath === '/' && href === '/') || (href !== '/' && currentPath.includes(href.split('/')[1]));
  };

  const renderDesktopNavLink = (href: string, title: string, icon: FAProSolid.IconDefinition, id?) => {
    const isCurrent = isCurrentLink(href);
    ...
    return (
      <Link className="group" href={href} passHref>
        <div
          ...
          <a className="flex items-center gap-2 group-hover:text-buttonMuted " href={href}>
            <FontAwesomeIcon
              icon={icon}
  ...
  const renderDesktopSideBar = () => {
    return (
      <div
        ...
        <div className="mx-auto flex w-fit items-center justify-center rounded bg-transparent">
          <Link href="/">
            <Image
              loader={contentfulLoader}
        ...
        <div className="mx-auto mb-20 mt-5 flex flex-col space-y-4">
          {renderDesktopNavLink('/discover', 'Discover', FAProSolid.faCompass)}
          {renderDesktopNavLink('/upload', 'Upload Audio', FAProSolid.faCloudArrowUp, 'click-to-upload')}
          {renderDesktopNavLink(`/player/${playerMediaKey?.toString() ?? 'demo'}`, 'Player', FAProSolid.faPodcast)}
          {publicKey !== null ? renderDesktopNavLink('/wallet', 'Wallet', FAProSolid.faWallet) : null}
          {currentCreatorHasUserAccount ? renderDesktopNavLink(getCreatorPlug(), 'Creator', FAProSolid.faUser) : null}
          {renderDesktopNavLink('/support/', 'Support', FAProSolid.faQuestionCircle)}
        </div>

        <LoginLogoutButton />

        <div className="left-5 mt-auto">
          <HexagonPFP />
          <p className="mt-2 flex flex-col text-sm font-black">
            <span id={E2EID.navbarBalance}>{balance === null ? '' : balance.toFixed(5)}</span>
        ...
        <div className={`static left-5 h-10 ${publicKey ? 'mt-3' : ''}`}>
          <p onClick={needHelp} className="mt-2 flex flex-col text-sm font-black">
            <span id="e2e-balance-status" className="cursor-pointer font-thin text-skin-muted underline">
              Need Help?
            </span>
          </p>
        </div>
  ...
  const renderMobileLink1 = (href: string, icon: IconDefinition, text: string) => {
    const isCurrent = isCurrentLink(href);
  ...
  const renderMobileNav1 = () => {
    return (
      <div className="fixed bottom-0 z-40 flex h-16 w-screen flex-row items-center justify-between space-x-4 overflow-hidden border-t-2 border-buttonAccentHover bg-transparent/60 backdrop-blur-md md:hidden">
        <div className="flex w-full justify-evenly">
          {renderMobileLink1(`/player/${playerMediaKey?.toString() ?? 'demo'}`, FAProSolid.faPlayCircle, 'Listen')}
          ...
          {currentCreatorHasUserAccount ? renderMobileLink1(getCreatorPlug(), FAProSolid.faUser, 'Creator') : null}
        </div>
      </div>
    );
  };
