...
import { getConfig, getTruncatedPublicKey } from '@utils';
import Image from 'next/image';
import Link from 'next/link';
import { useRouter } from 'next/router';
import { Fragment, useCallback, useEffect, useRef, useState } from 'react';
import Media from 'react-media';
import { tickFirebaseMediaPlaysUnsafe } from 'src/controllers/firebase/firebase-functions';
import { useAppState } from 'src/controllers/state/use-app-state';
import { printPlayerTime } from 'src/controllers/utils/time-helpers';
...
export function AudioPlayer() {
  ...
  }, [audioPlayer, previousTime, playCountTicked, duration, playerMediaKey, tickFirebaseMediaPlaysUnsafe]);
  ...
  }, [audioPlayer, playerAudioUrl, listenedDuration, playCountTicked, previousTime]);
    ...
    return (
      <>
        <div className="absolute left-16 top-4 max-w-[268px] md:top-3 md:max-w-md">
          <p className="text-sm font-bold truncate text-skin-base md:w-full md:text-base">{title}</p>
    ...
    return (
      <>
        <div className="absolute bottom-2 left-2 flex w-[90%] items-center justify-center gap-x-2">
          <p className="text-xs text-skin-muted">{printPlayerTime(currentTime)}</p>
          <input
          ...
          />
          <p className="text-xs text-skin-muted">{printPlayerTime(duration)}</p>
        </div>
      </>
    );
  };
